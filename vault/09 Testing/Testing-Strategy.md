...
**No test-DB reset exists**, and the seed data is exactly one car (the Toyota Camry, VIN `4T1B11HK5KU123456`) plus one demo `CONFIRMED` booking (2026-08-01..05) — both specs create their own fresh data every run: a timestamp+random unique email per registered account, and a date window per spec, randomized inside a **fixed** `minOffsetDays`..`spreadDays` range (`futureBookingWindow` in `e2e/support/test-data.ts`) disjoint from the other spec's own range and from the fixed demo booking. That range being fixed rather than shifting forward run over run means randomness alone does *not* prevent an eventual collision on the GiST exclusion constraint (Bookings.md/ADR-014) once enough leftover `CONFIRMED` bookings accumulate inside it — a real bug caught in review after a handful of cumulative runs. The actual fix: **both specs cancel the booking they created in `test.afterEach`** (a real `POST /api/bookings/:id/cancel` call, run whether the test passed or failed, via `e2e/support/api.ts`'s `cancelBooking` helper), so each run leaves its window exactly as it found it instead of leaking a `CONFIRMED` booking into it — repeat runs stay collision-free because the pool never grows, not because the random pick itself can't collide.
