...
- **Rationale:** No test-DB reset exists and the seed data is a single car — both new specs (customer journey, admin journey) create fresh, uniquely-timestamped accounts and a date window per spec randomized inside a fixed, mutually-disjoint `minOffsetDays`..`spreadDays` range, clear of the fixed seed demo booking, on the GiST exclusion constraint (ADR-014). The customer journey performs its own staff-side `POST /api/payments/:id/confirm-manual` call (via an isolated Playwright `request` context, separate from its own browser session) rather than depending on the admin spec's execution — the two spec files must each run independently, in any order. Neither "Add Car" nor "Approve Booking" has a real admin UI yet (cars module is GET-only; admin app has only `/sign-in` and `/dashboard`) — deliberately not fabricated; both are exercised as real authenticated API calls instead, documented as deferred until those UIs ship.
- **Status:** Accepted, amended same day after code review caught a real collision: since each spec's date-window range is fixed rather than shifting run over run, and neither spec cancelled the `CONFIRMED` booking it created, repeat local runs accumulated leftover bookings until a later run's random pick collided with one of them (a genuine `409` from ADR-014's constraint, not an API defect). Fixed by adding `test.afterEach` teardown to both specs that cancels the booking they created via a real `POST /api/bookings/:id/cancel` call (`e2e/support/api.ts`'s `cancelBooking` helper) — collision-free repeat runs now hold because each run leaves its window as it found it, not because the random pick alone can't collide. See `09 Testing/Testing-Strategy.md`'s "End-to-end tests" section for what's covered, what's deliberately skipped (booking pickup/return, rental completion, Add Car — none of these exist yet), and the exact commands.
