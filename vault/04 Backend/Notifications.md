...
**Sprint 9 fixes:**
- All three `PaymentSucceededEvent` listeners (this module's included) previously had no error handling of their own — since `emitAsync` uses `Promise.all` semantics, one listener throwing rejected the whole call back through every caller, silently blocking the other two listeners' side effects with no way to retry just the one that actually failed. Each listener now catches and logs its own errors (NestJS `Logger`) instead of rethrowing. See ADR-020 and `handle-payment-success.use-case.spec.ts` / `payment-succeeded.listener.spec.ts` (bookings) for the new test coverage this shipped with.
- Better Auth's password-reset flow had no email delivery wired up at all (`emailAndPassword` had no `sendResetPassword` callback) — `apps/api/src/modules/auth/auth.ts` now instantiates `ResendEmailSender` directly (outside Nest DI, matching that file's standalone-construction pattern) and calls it from `sendResetPassword`. Verified live: server log shows the same inert "Email not sent (Resend unconfigured)" debug line this module's other email sends already produce.
